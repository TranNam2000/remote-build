...
powershell -Command "if (-not (Get-NetTCPConnection -LocalPort 3000 -ErrorAction SilentlyContinue)) { Write-Host 'Starting build server...'; Start-Process -FilePath 'node' -ArgumentList 'backend\server.js' -WorkingDirectory '%~dp0' -WindowStyle Hidden; Start-Sleep 2; if (Get-NetTCPConnection -LocalPort 3000 -ErrorAction SilentlyContinue) { Write-Host 'Server running on port 3000' } else { Write-Host 'Server failed to start!'; exit 1 } } else { Write-Host 'Server already running on port 3000' }"
...
powershell -Command ^
    ...
    "$token = '8793252151:AAH-P7LoLGKKo5_pPBgk9MPlmVpOKsXPSN0'; " ^
    "$chatId = '2019979030'; " ^
    "$msg = \"*Remote Build Server Online!*`n`nhttps://%TUNNEL_HOSTNAME%`n`nURL co dinh.\"; " ^
    "try { Invoke-RestMethod -Uri \"https://api.telegram.org/bot$token/sendMessage\" -Method Post -Body @{chat_id=$chatId;parse_mode='Markdown';text=$msg} | Out-Null; Write-Host 'Sent to Telegram!' } catch {}"
...
powershell -ExecutionPolicy Bypass -Command ^
    "$proc = Start-Process -FilePath 'cloudflared' -ArgumentList 'tunnel','--url','http://localhost:3000' -RedirectStandardError '%TEMP%\tunnel.log' -PassThru -NoNewWindow; " ^
    "$found = $false; " ^
    "for ($i = 0; $i -lt 30; $i++) { " ^
    "    Start-Sleep -Seconds 2; " ^
    "    if (Test-Path '%TEMP%\tunnel.log') { " ^
    "        $content = Get-Content '%TEMP%\tunnel.log' -Raw -ErrorAction SilentlyContinue; " ^
